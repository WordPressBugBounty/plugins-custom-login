...
trait HooksTrait
{
     ...
     * @return string
     */
    protected function getWpFilterId(string $hook, $method, int $priority): string
    {
     ...
     */
    private function mapFilter(string $filter_id, $method, int $arg_count = 1): Closure
    {
        if (empty($this->filter_map[$filter_id])) {
            $this->filter_map[$filter_id] = static fn(...$args) => call_user_func_array(
                $method,
